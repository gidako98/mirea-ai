...
@app.post(
    ...
    tags=["quality"],
    summary="Вернуть полный набор флагов качества (включая флаги из HW03) по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    """
    Принимает CSV-файл и возвращает полный набор флагов качества.
    Важно: здесь должны быть видны ваши новые флаги из HW03.
    """
    ...
    # --- Важно: используем ядро из проекта (как в /quality-from-csv) ---
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags_all = compute_quality_flags(summary, missing_df)

    # jsonable_encoder приводит numpy/pandas типы к JSON-совместимым
    return {"flags": jsonable_encoder(flags_all)}
